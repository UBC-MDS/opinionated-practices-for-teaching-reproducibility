...
Think pair share exercises give students the opportunity to first reflect on their own
then pair up to discuss with a few classmates,
before optionally sharing with a larger group.
This pedagogy could be particulary effective in this context
...
However,
even with these more established tools
there can be significant upgrades
where we decide that the changes are wortwhile
to incorporate into our courses,
such as when R introcuced the native pipe symbol `|>`
...
e.g. switching to teaching a declrative visualization package in place of an imperative one.
...
that are triggered when new code is pused to a GitHub repository.
This has allowed us
