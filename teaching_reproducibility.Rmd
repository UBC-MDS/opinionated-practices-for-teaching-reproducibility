...
|-------------|-------|--------|
| Limitations in Excel data formats lead to the loss of 16,000 COVID case records in the Unied Kingdom. | ADD | CITE |
| Excel's automatic formatting caused important gene names to be disregarded in scientific studies. | ADD | @zeeberg2004mistaken; @ziemann2016gene |
| Deletion of a cell caused rows to shift and treatment groups to be mixed up | ADD | @wallensteen2018retraction |
| using binary instead of explanatory labels lead to a mix up of the intervention with the control group | ADD | CITE aboumatar/wise |
| Mixing up missing data and zero values  | Paper retraction  | CITE turcing and whitehouse |
| Incorrectly copying data in a spreadsheet | Delay in the opening a hospital | CITE Picken |
...
One way we have done this is providings students an analysis that is not 
reproducible 
(i.e., it depends on rare/obscure software pacakges 
...
run eachother's code projects that they have been working on in her course. 
...
with more homoegenous experiences.
...
and at present there are not a lot of clear and easy on roads for learners who
...
from master to main (rightfully so) [CAN WE CITE SOMETHING HERE OR LINK TO THEIR CHANGELOG?]. 
...
on collaboration with version control](https://ubc-dsci.github.io/introduction-to-datascience/version-control.html).
...
related to the the skills and tools they are practicing.
...
For the UBC Master of Data Science program students, for example, 
this ensures they are practicing using reproducible tools
...
"mechanical assignemnt requirements" worth a small percentage of their 
grade while completing assignments focused on other data science topics.
This latter strategy parrallels how they will employ reproducibility 
...
concepts behind reproducibility, but so that our teaching actually
changes their data analysis practices and their workflows in reproducible ones
